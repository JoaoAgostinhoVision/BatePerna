...
describe("MapaHome", () => {
  ...
  // COMPORTAMENTAL de propósito — uma leitura nova no contexto tem que mover os
  // dois. Um grep no fonte proibindo `useState` provaria menos e ainda vetaria
  // o `useVenceu`, que é o que mantém a hidratação honesta.
  it("leitura nova no contexto move o pin E o selo, no mesmo quadro", () => {
    ...
    const { container } = render(
      <LeiturasProvider value={nova}>
        ...
        <SeloTrilha slug={slug} inicial={leituras.get(slug)!} />
      </LeiturasProvider>,
    );
    // `leituras` (a prop, do servidor) diz "fresco"; o contexto diz "frio".
    // Os dois têm que obedecer ao contexto.
...
(acrescentar aos imports do arquivo: `import { LeiturasProvider } from "@/app/leituras";` e `import SeloTrilha from "@/app/SeloTrilha";`; `readFileSync`/`path` não são mais necessários aqui)
...
function montar(inicial: Record<string, LeituraCarimbo>) {
  return render(
    <HomeViva inicial={inicial}>
      <SeloTrilha slug="rampa" inicial={inicial.rampa} />
    </HomeViva>,
  );
}
...
describe("HomeViva", () => {
  it("sem provedor de leitura nova, o selo mostra o que veio do servidor", () => {
    montar({ rampa: { estado: "fresco", erro: false, calculadoEm: AGORA_S } });
    expect(screen.getByText("Pode subir")).toBeTruthy();
  });

  it("leitura vencida + volta pra frente = busca, e a tela troca palavra e cor juntas", async () => {
    ...
    expect(screen.getByText("Não suba")).toBeTruthy();
  });

  it("corpo fora do formato não vira decisão na tela", async () => {
    ...
    expect(screen.queryByText("Pode subir")).toBeNull();
    expect(screen.getByText("SEM INFORMAÇÕES")).toBeTruthy();
  });
