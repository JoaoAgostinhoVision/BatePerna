...
- Create: `src/app/CartaoTrilha.tsx`, `src/app/SeloTrilha.tsx`, `src/app/leituras.tsx`
...
export default function SeloTrilha({
  ...
  const venceu = carimboVenceu(leitura.calculadoEm, Math.floor(Date.now() / 1000));
...
> **`venceu` no primeiro render:** o servidor acabou de calcular, então `calculadoEm` é agora e `venceu` é `false` nos dois lados. A home é `force-dynamic` justamente pra isso continuar verdade.
...
.bp .selo[data-fase="sem-informacoes"] { --c: var(--stop-ink); --st-bg: var(--stop-bg); }
...
describe("MapaHome", () => {
  ...
  it("o pin e o selo pegam a cor da MESMA fonte, nunca de duas", () => {
    // Cicatriz: na rodada do carimbo o pin guardou a leitura do servidor e ficou
    // verde ao lado de um selo que já tinha virado vermelho. Os dois leem o
    // contexto de leituras; nenhum guarda estado próprio de cor.
    const fonte = readFileSync(path.join(process.cwd(), "src", "app", "PinTrilha.tsx"), "utf8");
    expect(fonte).toContain("useLeitura");
    expect(fonte).not.toMatch(/useState/);
  });
});
```
 ...
 *
 *  Nenhum useState aqui, de propósito. A cor sai do mesmo contexto que pinta o
 *  selo do cartão; estado próprio seria a segunda fonte de cor, e foi
 *  exatamente isso que uma vez pôs "Não suba" dentro de um selo verde. */
...
/* Sem leitura, o pin para de afirmar cor de veredito — igual ao selo. */
.bp .pin-home[data-fase="sem-informacoes"]::before { background: var(--stop); }
