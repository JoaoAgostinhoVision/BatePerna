...
/** O que está NA TELA agora, pro dono — não a fala de `falaMolhada` (essa é a
 *  consequência de ESCOLHER `frio` no formulário, uma coisa diferente). Se a
 *  leitura falhou, diz isso: `erro` nunca é mascarado, mesma regra de
 *  `carimbo-estado.ts`. */
function motorAgora(leitura: LeituraCarimbo): string {
  if (leitura.erro) return "Na tela agora: a leitura da chuva falhou.";
  return `Na tela agora: ${leitura.estado === "fresco" ? "Pode ir" : "Não vá"}`;
}
      ...
      {fichas.map((f) => {
        ...
        return (
          ...
          >
            ...
            {leitura && <p className="adm-motor">{motorAgora(leitura)}</p>}
